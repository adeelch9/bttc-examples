...
contract Uni {
    ...
    constructor(address account, address minter_, uint mintingAllowedAfter_) public {
        require(mintingAllowedAfter_ >= block.timestamp.div(1000), "Uni::constructor: minting can only begin after deployment");

        balances[account] = uint96(totalSupply);
        emit Transfer(address(uint160(0)), account, totalSupply);
        minter = minter_;
        emit MinterChanged(address(uint160(0)), minter);
    ...
    function mint(address dst, uint rawAmount) external {
        ...
        require(block.timestamp.div(1000) >= mintingAllowedAfter, "Uni::mint: minting not allowed yet");
        require(dst != address(uint160(0)), "Uni::mint: cannot transfer to the zero address");

        // record the mint
        mintingAllowedAfter = SafeMath.add(block.timestamp.div(1000), minimumTimeBetweenMints);
        ...
        emit Transfer(address(uint160(0)), dst, amount);

        // move delegates
        _moveDelegates(address(uint160(0)), delegates[dst], amount);
    }
    ...
    function permit(address owner, address spender, uint rawAmount, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(uint160(0)), "Uni::permit: invalid signature");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(uint160(0)), "Uni::delegateBySig: invalid signature");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(uint160(0)), "Uni::_transferTokens: cannot transfer from the zero address");
        require(dst != address(uint160(0)), "Uni::_transferTokens: cannot transfer to the zero address");
